...
class PoseMLP(nn.Module):
    ...
    def __init__(self, 
        ...
        self.fc_intrinsics = nn.Linear(output_dim, 4)
    ...
    def convert_intrinsic_to_4x4(self, B, out_k, device):
        intrinsic = torch.zeros((B, 4, 4), device=device)
        intrinsic[:, 0, 0] = out_k[:, 0]
        intrinsic[:, 1, 1] = out_k[:, 1]
        intrinsic[:, 0, 2] = out_k[:, 2]
        intrinsic[:, 1, 2] = out_k[:, 3]
        intrinsic[:, 2, 2] = 1.
        intrinsic[:, 3, 3] = 1.
        return intrinsic
    ...
    def forward(self, decout, img_shape):
        ...
        out_k = self.fc_intrinsics(feat)
        intrinsic = self.convert_intrinsic_to_4x4(B, out_k, tokens.device)
        res = {"pose": pose, "intrinsic": intrinsic}
        # res = {"pose": pose}
