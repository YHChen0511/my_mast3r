...
from mast3r.posemlp import PoseMLP  # noqa
...
class PoseHead(PixelwiseTaskWithDPT):
    ...
    def forward(self, decout, img_shape):
        ...
        # extract pose_features 
        pose_features = self.pose_dpt(decout, img_shape)
        ...
        if self.postprocess:
            out = self.postprocess(out,
                                   ...
                                   two_confs=self.two_confs,
                                   desc_conf_mode=self.desc_conf_mode)
        out["pose"] = pose_features["pose"]
        out["intrinsic"] = pose_features["intrinsic"]
        return out
...
def mast3r_head_factory(head_type, output_mode, net, has_conf=False):
    ...
    if head_type == 'catmlp+dpt' and output_mode.startswith('pts3d+desc'):
    ...
    elif head_type == 'pose_head' and output_mode.startswith('pose'):
        ...
        ed = net.enc_embed_dim
        dd = net.dec_embed_dim
        return PoseHead(net, local_feat_dim=local_feat_dim, has_conf=has_conf,
                                               num_channels=out_nchan + has_conf,
                                               feature_dim=feature_dim,
                                               last_dim=last_dim,
                                               hooks_idx=[0, l2 * 2 // 4, l2 * 3 // 4, l2],
